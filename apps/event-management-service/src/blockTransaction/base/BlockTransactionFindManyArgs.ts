/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { BlockTransactionWhereInput } from "./BlockTransactionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { BlockTransactionOrderByInput } from "./BlockTransactionOrderByInput";

@ArgsType()
class BlockTransactionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => BlockTransactionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => BlockTransactionWhereInput, { nullable: true })
  @Type(() => BlockTransactionWhereInput)
  where?: BlockTransactionWhereInput;

  @ApiProperty({
    required: false,
    type: [BlockTransactionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [BlockTransactionOrderByInput], { nullable: true })
  @Type(() => BlockTransactionOrderByInput)
  orderBy?: Array<BlockTransactionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { BlockTransactionFindManyArgs as BlockTransactionFindManyArgs };
